...
function NewSkillTreeGui:_setup()
	...
	local skill_export_button = self._panel:text({
		...
		text = managers.localization:to_upper_text("buildb_export_to_pd2skills"),
		align = "right",
		vertical = "top",
		font_size = tweak_data.menu.pd2_medium_font_size - 4,
		font = tweak_data.menu.pd2_medium_font,
...
function NewSkillTreeGui:mouse_pressed(button, x, y)
	...
	if button == Idstring("0") then
		if self._panel:child("export_skills_button"):inside(x, y) then
			Steam:overlay_activate("url", BuilDB:GetUrlFromSkills())
			return
...
function NewSkillTreeGui:mouse_moved(o, x, y)
	...
	if self._enabled then
		if self:check_buildb_button(x, y, "export_skills_button", "_skill_export_highlight") then
			inside = true
			pointer = "link"
