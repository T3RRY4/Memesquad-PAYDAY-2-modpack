...
local RusLocFixesLog = function(txt)
	log('[RusLocFixes] ' .. tostring(txt))
end

local data = {
		modworkshop_id = 24473,
		dl_url = 'https://bitbucket.org/andole/russian-localization-fixes/raw/updates/ruslocfixes-latest.zip',
		info_url = 'https://bitbucket.org/andole/russian-localization-fixes/raw/updates/info.json'
	}
	...
	if thisMod and not thisMod.updates_checked then
		...
		if not BLT.Downloads.the_fixes_clbk_dwnld_fin_no_ver then
			function BLT.Downloads:the_fixes_clbk_dwnld_fin_no_ver(data, http_id)
				...
				RusLocFixesLog(string.format("[Downloads] Finished download of %s (%s)", download.update:GetName(), download.update:GetParentMod():GetName()))
				...
				local save = function()
					...
					cleanup()
					RusLocFixesLog("[Downloads] Saving to downloads...")
					...
					RusLocFixesLog("[Downloads] Extracting...")
					download.state = "extracting"
					wait()
					unzip(file_path, temp_install_dir)
					wait()
					RusLocFixesLog("[Downloads] Going on unverified...")
					RusLocFixesLog("[Downloads] Removing old installation...")
					wait()
					...
					if not move_success then
						if jit and jit.os and not jit.os:lower():match('linux') then
							RusLocFixesLog("[Downloads] BLT failed to move the folder. Moving with Windows means...")
							os.execute('move "'..extract_path..'" "'..install_path..'"')
						else
							RusLocFixesLog("[Downloads] Failed to move installation directory!")
					...
					RusLocFixesLog("[Downloads] Complete!")
		...
		local function ParseInfo(text, id, message)
			...
			if message then
				...
				if success then
					if req_upd then
						dialog.text = 'Доступно обновление!'
						local bDownload = {
							text = 'Открыть загрузки',
						...
						dialog.text = 'Установлена последняя версия.'
					end
				else
					dialog.text = 'Не удалось проверить обновления.'
				end
