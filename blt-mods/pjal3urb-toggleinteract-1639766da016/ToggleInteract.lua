if RequiredScript == "lib/units/beings/player/states/playerstandard" then
	...
	PlayerStandard.EQUIPMENT_PRESS_INTERRUPT = false --Use the equipment key (default 'G') to toggle off active interactions
	PlayerStandard.VISUAL_LOCK = true	--Show interaction lock on HUD
...
elseif RequiredScript == "lib/managers/hudmanagerpd2" then
	...
	function HUDManager:set_interaction_locked(status)
...
elseif RequiredScript == "lib/managers/hud/hudinteraction" then
	...
	function HUDInteraction:show_interaction_bar(...)
		if PlayerStandard.VISUAL_LOCK then
			if not self._interact_circle_locked then
	...
	function HUDInteraction:hide_interaction_bar(...)		
		if self._interact_circle_locked then
			self._interact_circle_locked._circle:set_alpha(0)
		end
