...
function BlackMarketGui:_setup(is_start_page, component_data)
	...
	if not component_data then
		...
		if category == 'primaries' or category == 'secondaries' or category == 'masks' or component_data.topic_id == 'bm_menu_melee_weapons' then
			-- qued
		else
			dont_add_extra_button = true
		end
		...
		if component_data.buying_weapon or component_data.topic_id == 'bm_menu_buy_mask_title' then
			dont_add_extra_button = true
		end
	end
	...
	if _G.MoreWeaponStats and self._rweapon_stats_panel and self._tabs[self._selected]._data.identifier == self.identifiers.weapon then
...
function BlackMarketGui:si_refresh()
	...
	if tab then
		local qty
		if self._slot_data.category == 'masks' then
			qty = tweak_data.gui.MASK_ROWS_PER_PAGE * tweak_data.gui.MASK_COLUMNS_PER_PAGE
		else
			qty = tweak_data.gui.WEAPON_ROWS_PER_PAGE * tweak_data.gui.WEAPON_COLUMNS_PER_PAGE
		end

		local nr = (self._selected - 1) * qty
...
function BlackMarketGuiSlotItem:refresh()
	...
	if self.si_index and alive(self._panel) then
		if not self.si_text then
			self.si_text = managers.blackmarket:si_get_search_string(self._data, self.si_index)
		end

		self:si_match(SearchInventory.filters)
		local alpha = self.si_highlight and 1 or 0.1
		self._panel:set_alpha(alpha)
	end
end

function BlackMarketGuiSlotItem:si_match(filters)
	self.si_highlight = true
	if self.si_text and type(filters) == 'table' then
		for _, filter in ipairs(filters) do
			if not self.si_text:find(filter) then
				self.si_highlight = false
				break
			end
		end
	end
