...
function BlackMarketGui:mws_get_popup_data(equipped, remove_mod, add_mod, slot_data) -- mostly a copy paste from Better Weapon Stats
	...
	if tweak_data.weapon[slot_data.name] and slot_data.name ~= 'sentry_gun' then
		...
		data = {
			...
			fake_weapon_base = MoreWeaponStats.make_fakeweaponbase(factory_id, blueprint, name),
		}
...
function BlackMarketGui:mws_reload_full(data, index, txts)
	...
	local reload_mul = data.fake_weapon_base:reload_speed_multiplier()
...
function BlackMarketGui:mws_ammo_pickup(data, index, txts)
	...
	data.fake_weapon_base:add_ammo(1)
	...
	if pickup_range_l <= 0 and pickup_range_h <= 0 then
		...
		txts['a' .. index]:set_text(string.format('%.2f', pickup_range_l))
		txts['b' .. index]:set_text(string.format(' | %.2f', pickup_range_h))
	end
end

function BlackMarketGui:mws_falloff(data, index, txts)
	-- taken from BWS's function InventoryStatsPopup:_primaries_damage()
	if not table.contains(data.tweak.categories, 'shotgun') then
		txts['a' .. index]:set_text('')
		txts['b' .. index]:set_text('')
	else
		local near = data.fake_weapon_base._damage_near / 100
		local far = data.fake_weapon_base._damage_far / 100
		txts['a' .. index]:set_text(string.format('%.1fm', near))
		txts['b' .. index]:set_text(string.format(' | %.1fm', near + far))
	end
end
...
function MoreWeaponStats.make_fakeweaponbase(factory_id, blueprint, name_id)
	...
	local result = clone(_G[cls] or _G['NewRaycastWeaponBase'])
	...
	result.replenish = function() end

	local function swap_method(obj, orig, replacement, tmp)
		while true do
			if obj[replacement] then
				if tmp then
					obj[tmp] = obj[orig]
				end
				obj[orig] = obj[replacement]
			end
			obj = obj.super
			if not obj then
				break
	...
	swap_method(result, '_update_stats_values', 'mws_update_stats_values', 'mws_old_update_stats_values')
	result:_update_stats_values()
	swap_method(result, '_update_stats_values', 'mws_old_update_stats_values')
...
function MoreWeaponStats.make_playerstate(data)
	local fwb = data.fake_weapon_base
	return {
		_equipped_unit = {
			base = function()
				return fwb
...
function MoreWeaponStats.mws_get_spread(data, st_moving, st_ducking, st_deploy, st_steelsight)
	...
	local spread_x, spread_y = data.fake_weapon_base:_get_spread(fake_user_unit)
	return spread_x
end

function BlackMarketGui:mws_accuracy(data, index, txts, st_ducking, st_deploy, st_steelsight)
	if table.contains(data.tweak.categories, 'saw') then
		txts['a' .. index]:set_text('')
		txts['b' .. index]:set_text('')
	else
...
function MoreWeaponStats.mws_get_recoil(data, st_ducking, st_steelsight)
	-- comes from function PlayerStandard:_check_action_primary_attack()
	local recoil = data.fake_weapon_base._current_stats.recoil
	local indices = data.fake_weapon_base._current_stats_indices
	local recoil_addend = managers.blackmarket:recoil_addend(data.name, data.tweak.categories, indices and indices.recoil, data.silencer, data.blueprint)
...
function BlackMarketGui:mws_recoil_horiz(data, index, txts)
	local recoil = MoreWeaponStats.mws_get_recoil(data, false, false)
...
function MoreWeaponStats.mws_get_swap_speed(data)
	local fake_player_state = MoreWeaponStats.make_playerstate(data)
	...
	local equip = (data.tweak.timers.equip or 0.7) / divider

	-- 0.5 found in PlayerStandard:_start_action_unequip_weapon()
	local unequip = (data.tweak.timers.unequip or 0.5) / divider

	return equip, unequip
end

function BlackMarketGui:mws_equip_delay(data, index, txts)
	local equip, unequip = MoreWeaponStats.mws_get_swap_speed(data)
	txts['a' .. index]:set_text(string.format('%.2fs', equip))
	txts['b' .. index]:set_text(string.format(' | %.2fs', unequip))
end
...
function MoreWeaponStats:get_breakpoints(unit_type, params, shoot_interval)
	local result = {}
	...
	if params.trigger_happy then
		...
		th_nr = math.floor(td.max_time / shoot_interval)
	end
	...
	for i = 1, _HEALTH_GRANULARITY do
		...
		local final_i = i
		-- if th_mul and crit_nr > 0 then
			-- not supported yet
		if th_mul then
			if i > 1 then
				local th_initiator_nr = math.floor(i / th_nr)
				local th_boosted_nr = math.max(0, i - th_initiator_nr)
				final_i = final_i + th_boosted_nr * (th_mul - 1)
			end
...
function BlackMarketGui:mws_update_breakpoints()
	...
	if data then
		...
		local can_full_auto = data.tweak.FIRE_MODE == 'auto' or data.tweak.CAN_TOGGLE_FIREMODE
		local shoot_interval = data.fake_weapon_base._next_fire_allowed
		if not can_full_auto then
			shoot_interval = math.max(1/6, shoot_interval) -- max of 6 clicks per sec
		end
		...
		local body_expertise = bonuses.body_expertise.checked and (can_full_auto and weapon_in_category(data.categories, 'smg', 'lmg', 'assault_rifle', 'minigun') or weapon_in_category(data.categories, 'bow', 'saw'))
		...
		local breakpoints_data = MoreWeaponStats:get_all_breakpoints(params, shoot_interval)
...
function MoreWeaponStats:get_all_breakpoints(params, shoot_interval)
	local result = {}
	for _, unit_type in pairs(self.unit_types) do
		result[unit_type] = self:get_breakpoints(unit_type, params, shoot_interval)
	end
	return result
end
